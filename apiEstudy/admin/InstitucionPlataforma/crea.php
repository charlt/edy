...
$ruta = htmlspecialchars('../../../assets/institucion/');
...
$directorio2 = $directorio.'/imagen_perfil';
     if( !is_dir($directorio2) )
    {
        $crear = mkdir($directorio2);
        if(!$crear){
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            ...
            $inserta = inserta('plataforma_institucion',
            'institucion_id,color,url_perfil, estatus', ''.$usuario->id_institucion.',"'.$color.'",
            "'.$res.'",1');

            if($inserta){
                $json = array("status" => 1, "msg" => "Foto de perfil se guardó correctamente.");
