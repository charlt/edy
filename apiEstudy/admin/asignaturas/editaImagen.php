  ...
  if($_SERVER['REQUEST_METHOD'] == "POST"){
    foreach($_POST as $clave => $valor){
      ...
      $imagen = $archivo::guardar_archivo(
          "asignaturas/carga_imagen", // la ruta del archivo
          $id_asignatura, //el id de la ruta
          $_FILES["imagen"],//la variable tipo file donde viene el archivo
          "img_asignatura", //el nombre que va a llevar el archivo a guardar
          1, //tipo de archivos 
          null //tamaño de archivo
        );

                    
        //la función guarda archivo devuelve un status, que para mostrar un mensaje
        //en el formato requerido lo enviamos a mensaje estatus
        //si status es diferente de uno el archivo no se guardo correctamente
        if($imagen['status'] == 1){
          update('cat_asignaturas','url_imagen = "'.$imagen['url'].'"','asignatura_id = '.$id_asignatura);
