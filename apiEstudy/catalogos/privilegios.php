...
try {
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET"){
    foreach($_GET as $clave => $valor){
      ...
      if(isset($muestra_estatus))
        $script_status = '' ;
      else
        $script_status = 'cp.estatus=1 and' ;
        
      
    //SERVICIO DE CONSULTA DE SESIÓN
    $query_privilegios = query('SELECT cp.privilegio_padre_id,cp.privilegio_id, cp.privilegio_clave, cp.privilegio,icono, cp.url, cp.estatus
    FROM  cat_privilegios cp 
    WHERE  '.$script_status.' privilegio_padre_id is null');
    ...
    while ($arreglo = arreglo($query_privilegios)){
      ...
      FROM  cat_privilegios cp 
      WHERE '.$script_status.' privilegio_padre_id = '.$arreglo['privilegio_id']);
