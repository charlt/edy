...
try {
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET"){
    ...
    $query_asistencia = query('
    SELECT 
    a.clave_alumno, p.nombre, p.primer_apellido, p.segundo_apellido, a.alumno_id , a1.*
    FROM tr_alumno a
    join '.$GLOBALS['db_datosGenerales'].'.personas p on p.persona_id = a.alumno_id
    join asistencia a1 ON a.alumno_id = a1.alumno_id
    where asignatura_grupo_id = '.$id_asignatura_grupo.' and fecha = "'.$fecha.'"
    ');
