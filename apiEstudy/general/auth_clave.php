...
try {
  if(isset($_POST['clave']) && isset($_POST['rol'])  ){
    ...
    if($rol == 2){
      ...
      join usuarios u on u.usuario_id = p.persona_id
      ...
      $query_usuario = query('SELECT nombre, primer_apellido,segundo_apellido, p.persona_id,curp,
        ...
        join usuarios u on u.usuario_id = p.persona_id
        JOIN cat_rol cr ON cr.rol_id = ur.rol_id
        where ta.clave_profesor = "'.$clave.'" and ur.rol_id = '.$rol.'  and u.estatus = 1');
    ...
    if($res){
      ...
      if($arreglo_usuario['rol_id'] == 3){
        $query_docente = query('select docente_id,clave_profesor,situacion_docente_id,activa_chat,edicion_contenido from '.$GLOBALS['db_controlEscolar'].'.tr_docente where clave_profesor = "'.$clave.'"');
