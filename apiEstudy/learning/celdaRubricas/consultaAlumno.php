...
try {
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET"){
    ...
    //SERVICIO DE CONSULTA DE SESIÓN
      ...
      ta.valor, ta.respuesta_id,ird.docente_id
      FROM tr_rubrica tr
      ...
      left join inter_respuesta_docente ird on ird.respuesta_id = ta.respuesta_id
      WHERE tr.rubrica_id = '.$id_rubrica.' and ird.materia_fecha_actividad_id ='.$id_materia_fecha_actividad.' and tr.estatus=1');

    
       $json_respuesta= array();
       while ($arreglo_respuesta = arreglo($query_respuesta)){

        if(isset($arreglo_respuesta['docente_id'])){
          $arreglo_respuesta['seleccionado'] = 1;
        }else{
          $arreglo_respuesta['seleccionado'] = 0;
        }
