        ...
        $usuario = Auth::GetData(
          ...
          for ($i=0; $i <$num_registros ; $i++) { 
              if (isset($id_elemento[$i])) {
                $edita = update('tr_cc_elementos','elemento_desc="'.$elemento_desc[$i].'",
                fecha_actualiza= now()', 'elemento_id='.$id_elemento[$i].'');
                
              }else{
              $inserta = inserta('tr_cc_elementos','respuesta_id, elemento_desc, fecha_creacion, estatus',
              ''.$id_respuesta.',"'.$elemento_desc[$i].'", now(), 1');           
              }
            }
