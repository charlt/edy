...
try {
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET"){
    ...
    $usuario = Auth::GetData(
        ...
        $elimina_respuesta_finales = query('delete trf FROM tr_cc_respuesta tr
        ...
        $elimina_elemento = query('delete from tr_cc_elementos 
        where respuesta_id = '.$id_respuesta);
        if($elimina_elemento){
            
        $elimina_categoria = query('delete  from tr_cc_categorias
        where respuesta_id = '.$id_respuesta);

        if($elimina_categoria){
            
            $elimina_pregunta = query('delete from tr_cc_respuesta
            where respuesta_id = '.$id_respuesta);
            }
        }
        }
