    ...
    $usuario = Auth::GetData(
      ...
      for ($i=0; $i <$num_registros ; $i++) { 
          if(isset($id_categoria[$i])){
            $edita = update('tr_rb_categoria', 'categoria_desc="'.$categoria_desc[$i].'",
             fecha_actualiza= now()','categoria_id='.$id_categoria[$i].'');
                   
          }else{
            $inserta = inserta('tr_rb_categoria','categoria_desc, rubrica_id, fecha_creacion, estatus',
            '"'.$categoria_desc[$i].'",'.$id_rubrica.', now(), 1');  
          }
        }
