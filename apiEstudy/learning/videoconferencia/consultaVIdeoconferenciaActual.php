...
try {
  ...
  if($_SERVER['REQUEST_METHOD'] == "GET"){
    foreach($_GET as $clave => $valor){
      ...
      $query = query('SELECT tm.asignatura_grupo_id,videoconferencia_id,usuario_zoom_id,nombre,descripcion,tv.fecha_inicio,tv.fecha_fin FROM tr_videoconferencia tv
      join tr_materia tm on tm.asignatura_grupo_id = tv.asignatura_grupo_id      
      WHERE materia_id  =  '.$id_materia.'  and tv.estatus!=0 and now() between tv.fecha_inicio and tv.fecha_fin
