...
$id_solicitud_pago = $_GET['id_solicitud_pago'];
...
function Header()
{
    ...
    $this->Image($GLOBALS['url_front'].'assets/images/logo.png',15 ,15, 30, 28);
    $this->SetFont('Arial','B',10);
    // Movernos a la derecha
    $this->SetXY(105,15);
    $this->Cell(30,8,$GLOBALS['nombre_institucion'], 0, 0,'L');
    // Salto de línea
    $this->Ln(20);
}
...
$arreglo = arreglo(query('select tsp.solicitud_pago_id,p.nombre,p.primer_apellido, p.segundo_apellido,
tsp.monto_final,cfp.descripcion, tsp.nombre_pago as descripcion_pago, DATE_FORMAT(now(), "%d/%m/%Y") as fecha,
tsp.observacion
from tr_solicitud_pago tsp
join tr_pago cp on cp.pago_id = tsp.pago_id 
...
where tsp.solicitud_pago_id = '.$id_solicitud_pago.' and tsp.estatus_solicitud_pago_id = 3'));
...
$pdf->SetXY(50,33);
$pdf->Cell(12,6,'No. '.$arreglo['solicitud_pago_id'],0,0,'C', 0);
...
$pdf->SetXY(50,66);
$pdf->MultiCell(95,5, utf8_decode($arreglo['descripcion_pago'].""),0,'L');    
$pdf->SetXY(145,66);
$pdf->MultiCell(25,10, utf8_decode("$".$arreglo['monto_final'].""),0,'L');    
$pdf->SetXY(170,66);
...
if(!empty($arreglo['observacion'])){
    $pdf->SetX(15);
    $pdf->Cell(26,8, utf8_decode("Observaciones:"),1,0,'L', 0);    
    $pdf->Cell(156,8, utf8_decode($arreglo['observacion']),1,1,'L', 0);    
}
$pdf->SetX(15);
$pdf->Cell(182,2, utf8_decode(""),0,1,'L', 0);    
$pdf->SetX(15);
...
function basico($numero) {
    ...
    'nueve','diez','veinte', 'veinticuatro','veinticinco',
    ...
    $div_num= explode(".",$arreglo['monto_final']);
...
$pdf->SetFont('Arial','B',10);
$pdf->SetXY(105,160);
$pdf->Cell(30,8,'IGM Plantel Sur', 0, 0,'L');
...
$pdf->Cell(182,8, utf8_decode("Firma y Sello"),0,0,'C', 0);    
    ...
    $pdf->Cell(182,8, utf8_decode("No se encontro Pago"),0,0,'C', 0);    
}
