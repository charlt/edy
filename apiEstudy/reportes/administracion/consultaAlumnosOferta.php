...
try {
  ...
  $query = query('SELECT carrera_id, carrera_clave, carrera from '.$GLOBALS['db_datosGenerales'].'.tr_carrera where estatus = 1');
   ...
   $fila_alumno =5;
    ...
    while($arreglo =  arreglo($query)){
      ...
      $objPHPExcel->getActiveSheet()->setCellValue('D'.$fila_valores,'Egresado');            
          ...
          (select count(ta.alumno_id) as num_alumnos, situacion_alumno_id  from tr_alumno ta 
          ...
          where tpe.carrera_id = '.$arreglo['carrera_id'].' and ta.estatus = 1  group by situacion_alumno_id) t1
