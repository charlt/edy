...
try {
  ...
  foreach($_GET as $clave => $valor){
    ${$clave} = escape_cara($valor);
      ...
      left join '.$GLOBALS['db_datosGenerales'].'.cat_campus cc on cc.campus_id = iap.campus_id and cc.campus_id = 1
      where ced.estatus =1 and cd.estatus=1 and ta.estatus=1 and
      ie.estatus=1 and cd.estatus =1 and p.estatus=1 order by nombre, primer_apellido, segundo_apellido');
