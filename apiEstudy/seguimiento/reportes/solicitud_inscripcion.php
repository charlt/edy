...
$query_curso = query('select tc.carrera, tc.carrera_id from  '.$GLOBALS['db_datosGenerales'].'.tr_carrera tc join tr_aspirante ta on ta.carrera_id = tc.carrera_id  where aspirante_id='.$id_aspirante);
...
$query = query('SELECT a.aspirante_id as clave_alumno, clave_aspirante,nombre, primer_apellido, segundo_apellido, 
curp, rfc, fecha_nacimiento, p.email, sexo_id, otros_datos_id,color, p.fecha_nacimiento, p.celular
FROM tr_aspirante a
join '.$GLOBALS["db_datosGenerales"].'.personas p on a.aspirante_id = p.persona_id 
...
WHERE a.aspirante_id = '.$id_aspirante.' and rol_id = 4');
$formulario=array();
...
right join inter_campo_aspirante ica on ica.campo_formulario_id = tcf.campo_formulario_id
    where ica.aspirante_id ='.$id_aspirante);
