...
export class FormPagoComponent implements OnInit {
      ...
      this._pagoService.consultaSolicitud(this.idSolicitudPago).subscribe(
        response=>{
          if(response && response['status']){
            ...
            this.info_pago = solicitud.info_pago;
            this.idAlumno = solicitud.persona_id;
            this.monto = solicitud.monto;
            this.beca = solicitud.beca ? solicitud.beca : 0;
  ...
  aplicaBeca(event){
    ...
    let montoFinal = this.monto - this.beca;
  ...
  aplicaDescuento(event){
    ...
    let montoPago = this.monto - this.beca;
    ...
    let montoFinal = montoPago - this.montoDescuento;
  ...
  aplicaRecargo(){
    let montoFinal = this.monto - this.beca - this.montoDescuento + parseFloat(this.solicitudForm.get('recargo').value);
